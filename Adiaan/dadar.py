...
# --- CSS: MANAGE APP, SHARE FI DEPLOY DHOKSUUF ---
# Koodiin kun hunda isaanii iddoo tokkotti dhoksa
st.markdown("""
    <style>
    /* 1. Header gubbaa (Manage app, Share, Deploy) guutummaatti haquuf */
    header, .stAppToolbar, .stActionButton, .stDeployButton {
        visibility: hidden; 
        display: none !important;
    }
    
    /* 2. Menu sarara sadii dhoksuuf */
    #MainMenu {visibility: hidden;}
    
    /* 3. Footer 'Made with Streamlit' dhoksuuf */
    footer {visibility: hidden;}
    
    /* 4. Iddoo duwwaa gubbaa hir'isuuf */
    .block-container {
        padding-top: 1rem;
        padding-bottom: 0rem;
    }

    /* 5. Halluu Sidebar fi Card (Style kee isa duraa) */
...
if not st.session_state.logged_in:
    ...
    with col_mid:
        ...
        st.markdown("<h2 style='text-align:center;'>Dadar Land Administration</h2>", unsafe_allow_html=True)
        ...
        with st.form("entry_form", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Galmeessi"):
                if m_name and ogeessa:
                    new_row = [datetime.now().strftime('%d/%m/%Y'), m_name, m_ara, "Qaxana", "Tajaajila", ogeessa, kafaltii]
