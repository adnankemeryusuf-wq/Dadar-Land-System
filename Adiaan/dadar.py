...
def generate_chart(df):
    fig, ax = plt.subplots()
    df.groupby("Maqaa_Ogeessa")["Kafaltii"].sum().plot(
        kind="bar",
        ax=ax
    )
...
with st.sidebar:
    ...
    menu = st.radio(
        "MENU",
        ["📊 Dashboard", "📝 Galmee Haaraa", "📈 Gabaasa", "🏆 Badhaasa", "🚪 Ba'i"]
    )
...
if menu == "📊 Dashboard":
    if df.empty:
        ...
        c1.markdown(
            f"<div class='card'><h3>💰 Galii</h3><h2>{df['Kafaltii'].sum():,.2f}</h2></div>",
            unsafe_allow_html=True
        )
        c2.markdown(
            f"<div class='card'><h3>👥 Tajaajilamtoota</h3><h2>{len(df)}</h2></div>",
            unsafe_allow_html=True
        )
        c3.markdown(
            f"<div class='card'><h3>👷 Ogeeyyii</h3><h2>{df['Maqaa_Ogeessa'].nunique()}</h2></div>",
            unsafe_allow_html=True
        )
