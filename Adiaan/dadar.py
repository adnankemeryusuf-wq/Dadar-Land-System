...
div.stForm { background:white;border-radius:15px;padding:25px;border:2px solid #2e7d32;box-shadow:0 4px 15px rgba(0,0,0,0.1);}
...
def send_report_telegram(df):
    ...
    fig.write_image(chart_buf,format='png'); chart_buf.seek(0)
...
def create_pdf_certificate(name,count,rank):
    ...
    pdf.set_draw_color(*rank_color); pdf.set_line_width(3); pdf.rect(10,10,277,190)
    pdf.set_font("Arial","B",30); pdf.cell(0,30,"SARTIIFIKETA BEEKAMTII",ln=True,align="C")
    pdf.set_font("Arial","B",22); pdf.cell(0,20,name.upper(),ln=True,align="C")
    pdf.set_font("Arial","",16); pdf.multi_cell(0,10,f"Waggaa 2026 tajaajila {count} kennuun sadarkaa {rank}ffaa argateera.",align="C")
...
if menu=="📊 Dashboard":
    ...
    fig=px.bar(df.groupby("Maqaa_Ogeessa")["Kafaltii_Taj"].sum().reset_index(),
               x="Maqaa_Ogeessa",y="Kafaltii_Taj",
               color="Kafaltii_Taj",color_continuous_scale="Greens",
               text="Kafaltii_Taj",title="Galii Ogeeyyii")
...
# ---------- MULTI-SERVICE REGISTRATION ----------
    ...
    GATII_DICT={"Gibira":["Gibira Baaxii","Gibira Qonnaa"],
...
# ---------- REPORT ----------
    ...
    if not df.empty:
        ...
        if st.button("✈️ Telegramitti Ergi"): send_report_telegram(df); st.success("✅ Telegram ergameera!")
