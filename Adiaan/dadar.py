...
def create_clearance_pdf(data):
    ...
    pdf.write(5, "Lakk. Galmee: DAD/WL/2018/____"); pdf.set_x(140)
    pdf.write(5, f"Guyyaa: {guyyaa_ec}"); pdf.ln(18)
    pdf.set_font('Times', 'B', 14); pdf.cell(0, 10, "WARAQAA RAGAA QULQULLINAA", ln=True, align='C'); pdf.ln(8)
    pdf.set_font('Times', '', 12); pdf.set_x(20)
    pdf.write(9, f"Waraqaan kun Obbo/Adde {data['maqaa']} Araddaa {data['araddaa']} Lakk. Kaartaa {data['kaartaa']} qabaniif kenname.\n")
    pdf.write(9, f"Dhimma: {data['dhimma']}. Dhorkaa kamirrayyuu bilisa ta'uu mirkaneessina.\n")
    pdf.set_y(235); pdf.set_x(20); pdf.write(8, f"Itti Gaafatamaa: {data['head_name']}\nMallattoo: _________________")
    return pdf.output(dest='S').encode('latin-1')
...
if not st.session_state.logged_in:
    ...
    with col:
        ...
        with st.form("login"):
            ...
            if st.form_submit_button("SEENI"):
                ...
                else: st.error("Odeeffannoon dogoggora!")
else:
    df = load_data()
    with st.sidebar:
    ...
    if menu == "📊 Dashboard":
        ...
        if not df.empty:
            ...
            c3.markdown(f"<div class='card'><p>🏆 Ogeessa Cimaa</p><p class='metric-value' style='font-size:1.5rem;'>{top_og}</p></div>", unsafe_allow_html=True)
        ...
        with st.form("reg_form"):
            ...
            taj = c1.selectbox("Gosa Tajaajilaa", ["Kaartaa Haaraa", "Jijjiirraa Maqaa", "Gibira Waggaa", "Liizii"])
            ogeessa = c2.text_input("Maqaa Ogeessaa")
            fee = st.number_input("Kaffaltii (ETB)", min_value=0.0)
            if st.form_submit_button("💾 GALMEESSI"):
                if name:
                    new_row = [datetime.now().strftime('%d/%m/%Y %H:%M'), name, ara, "-", taj, ogeessa, fee]
        ...
        if 'pdf_to_download' in st.session_state:
            ...
            with st.form("clearance"):
                c1, c2 = st.columns(2)
                m = {'maqaa': c1.text_input("Maqaa"), 'araddaa': c2.text_input("Araddaa"), 'kaartaa': c1.text_input("Kaartaa"), 'dhimma': c2.selectbox("Dhimma", ["Gurgurtaa", "Liqii"]), 'head_name': st.text_input("Itti Gaafatamaa")}
        ...
        if not df.empty:
            ...
            st.info(f"Ogeessi Cimaa: {top_og}")
            if st.button("📄 Sartiifikeeta Uumi"):
                st.download_button("📥 Buufadhu", create_pdf_cert(top_og, len(df), 1, None, None), "Badhaasa.pdf")
