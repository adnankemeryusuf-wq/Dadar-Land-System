...
def create_clearance_pdf(data):
    ...
    pdf.cell(90, 5, f"Lakk. Galmee: DAD/WL/{now_ec[0]}/____", ln=False, align='L')
    ...
    k_ibsa = "2. Kaffaltii Liizii waggaa/duraa kan kaffalamuu qabu hunda kaffalanii kan xumuran ta'uu isaanii ni mirkaneessina." if data['gosa_qabiyyee'] == "Liizii" else "2. Kaffaltii tajaajilaa fi kaffaltiiwwan adda addaa qabiyyee durii kanaan wal qabatan hunda raawwatanii kan xumuran ta'uu isaanii ni mirkaneessina."
    ...
    pdf.cell(0, 7, "Maqaa Itti Gaafatamaa: ________________", ln=True)
...
up_b = st.sidebar.file_uploader("Logo Bittaa", type=['png', 'jpg', 'jpeg'])
if up_b: Image.open(up_b).convert("RGB").save("logo_bitta.jpg", "JPEG")
up_m = st.sidebar.file_uploader("Logo Mirgaa", type=['png', 'jpg', 'jpeg'])
if up_m: Image.open(up_m).convert("RGB").save("logo_mirga.jpg", "JPEG")

menu = st.sidebar.radio("FILANNOO", ["📝 Galmee & Clearance", "📊 Dashboard", "📈 Gabaasa"])

if menu == "📝 Galmee & Clearance":
    st.header("📝 Galmee Tajaajilaa fi Clearance")
    ...
    with st.form("main_form"):
        ...
        m_kafaltii = st.number_input("Waliigala Kaffaltii (ETB)", min_value=0.0)
        m_dhorkaa = st.checkbox("Qabiyyeen kun Dhorkaa irraa bilisa ta'uu nan mirkaneessa.")
        
        if st.form_submit_button("💾 GALMEESSI FI PDF UUMI"):
            if m_maqaa and m_araddaa and m_dhorkaa:
                # Save Data
                new_row = [datetime.now().strftime('%d/%m/%Y'), m_maqaa, m_araddaa, m_qaxana, m_dhimma, m_ogeessa, m_kafaltii]
                ...
                st.rerun()
            else: st.error("Maaloo odeeffannoo barbaachisaa guuti!")

elif menu == "📊 Dashboard":
    st.title("📊 Gabaasa Gabaabaa")
    c1, c2 = st.columns(2)
