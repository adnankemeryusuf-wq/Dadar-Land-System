    ...
    if menu == "📝 Galmee Haaraa":
        ...
        with st.form("entry_form", clear_on_submit=True):
            if is_tot_selected:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    maqaa_g, araddaa_g, qaxana_g = st.text_input("Maqaa Gurguraa"), st.text_input("Araddaa (G)"), st.text_input("Qaxana (G)")
                with c2:
                    maqaa_b, araddaa_b, qaxana_b = st.text_input("Maqaa Bitataa"), st.text_input("Araddaa (B)"), st.text_input("Qaxana (B)")
                maqaa_final, araddaa_final, qaxana_final = f"G: {maqaa_g} / B: {maqaa_b}", f"G: {araddaa_g} / B: {araddaa_b}", f"G: {qaxana_g} / B: {qaxana_b}"
            else:
                c1, c2 = st.columns(2)
                maqaa_final, araddaa_final = c1.text_input("Maqaa Abbaa Dhimmaa"), c2.text_input("Araddaa")
                qaxana_final = c1.text_input("Qaxana")
            
            ogeessa = st.text_input("Maqaa Ogeessaa")
            if st.form_submit_button("💾 Galmeessi"):
                if maqaa_final and ogeessa and details_list:
                    new_row = [datetime.now().strftime('%d/%m/%Y'), maqaa_final, araddaa_final, qaxana_final, ", ".join(details_list), ogeessa, sum(dynamic_fees.values())]
    ...
    # --- 📈 GABAASA BAL'AA ---
        ...
        if not df.empty:
            ...
            if f_type == "Kurmaana":
                ...
                sel_m, sel_w = st.sidebar.selectbox("Ji'a ", list(MONTH_MAP.values())), st.sidebar.selectbox("Torbee", [1, 2, 3, 4])
                filtered_df = filtered_df[(filtered_df['Ji\'a'] == sel_m) & (filtered_df['Torbee'] == sel_w)]
            elif f_type == "Guyyaa":
                sel_d = st.sidebar.selectbox("Guyyaa", ["Wixata", "Filatama", "Roobii", "Kamisa", "Jimmata"])
            ...
            with c2:
                if st.button("✈️ Telegram-itti Ergi"):
                    ...
                    if send_to_telegram(buffer.getvalue(), f"Gabaasa_{f_type}.xlsx", caption): st.success("✅ Ergameera!")
        ...
        if search_q:
            ...
            for idx, row in results.iterrows():
                with st.expander(f"📄 {row['Maqaa_Abbaa_Dhimmaa']}"):
                    ...
                    with c_d:
                        if st.button("🗑️ Haqi (Delete)", key=f"d_{idx}"):
