...
def create_clearance_pdf(data):
    ...
    pdf.ln(10); pdf.set_font('Arial', '', 12)
    body = f"Obbo/Adde {data['maqaa']} Araddaa {data['araddaa']} Lakk. Kaartaa {data['kaartaa']} qabaniif dhimma {data['dhimma']} kaffaltii bara {data['bara_gibiraa']} xumuruu isaanii ni mirkaneessina."
...
if not st.session_state.logged_in:
    ...
    if menu == "📊 Dashboard":
        ...
        if not df.empty:
            ...
            c3.markdown(f"<div class='card'><p>🏆 Ogeessa Cimaa</p><p class='metric-value' style='font-size:1.2rem;'>{df['Maqaa_Ogeessa'].mode()[0]}</p></div>", unsafe_allow_html=True)

    elif menu == "📝 Galmee Tajaajilaa":
        st.header("📝 Galmee Haaraa")
        GATII_DICT = {
            "🏷 Gibira & Kaffaltii": ["Gibira Baaxii Gooroo", "Kaffaltii Liizii", "TOT (Turnover Tax)"],
            "📜 Kaartaa": ["Kaartaa Haaraa", "Jijjiirraa Maqaa (Sale)"],
        ...
        if sel_main:
            for g in sel_main:
                ...
                for s in subs:
                    fee = st.number_input(f"Gatii {s}", min_value=0.0)
                    details.append(s); d_fees[s] = fee
                    if "TOT" in s or "Sale" in s: is_tot = True
        with st.form("reg_form"):
            ...
            if st.form_submit_button("💾 GALMEESSI"):
                ...
                new_row = [datetime.now().strftime('%d/%m/%Y %H:%M'), name, ara, "-", ", ".join(details), ogeessa, total]
        ...
        st.header("📜 Clearance")
        with st.form("c_form"):
            m_data = {'maqaa': st.text_input("Maqaa"), 'araddaa': st.text_input("Araddaa"), 'kaartaa': st.text_input("Kaartaa"), 'bara_gibiraa': st.text_input("Bara Gibiraa"), 'dhimma': st.text_input("Dhimma"), 'head_name': st.text_input("Itti Gaafatamaa")}
            if st.form_submit_button("📄 PDF UUMI"):
                st.download_button("📥 Buufadhu", create_clearance_pdf(m_data), "Clearance.pdf")
        ...
        if not df.empty:
            ...
            for i, (name, count) in enumerate(top_3.items()):
                with cols[i]:
                    ...
                    if st.button(f"Cert Uumi {i+1}"):
                        st.download_button("📥 Download", create_pdf_cert(name, count, i+1, l_up, r_up), f"Cert_{name}.pdf")
