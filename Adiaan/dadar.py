...
# Galmee Excel load gochuu
df = pd.read_excel("Abbaa Adeemsaa.xlsx", sheet_name=None)

# Language option
lang = st.sidebar.selectbox("Afaan filadhu / Choose Language", ["Afaan Oromoo", "English"])

# Text labels bilingual
if lang == "Afaan Oromoo":
    st.title("Madaallii Raawwii Hojii Hojjettoota")
    sheet_label = "Sheet filadhu"
    worker_label = "Hojjetaa filadhu"
    score_label = "Qabxii Madaallii"
    submit_label = "Galmee Qabxii"
else:
    st.title("Performance Evaluation Dashboard")
    sheet_label = "Select Sheet"
    worker_label = "Select Worker"
    score_label = "Evaluation Score"
    submit_label = "Submit Score"

# Sidebar keessatti sheet filachuuf
sheet_names = list(df.keys())
selected_sheet = st.sidebar.selectbox(sheet_label, sheet_names)

# Data agarsiisu
st.subheader(f"{selected_sheet} Data")
st.dataframe(df[selected_sheet])

# Fakkeenya chart (yoo kolumniin jiru ta’e)
sheet_df = df[selected_sheet]
if {'Ulfa','U/Raw'}.issubset(sheet_df.columns):
    st.line_chart(sheet_df[['Ulfa','U/Raw']])

# Filter hojjetaa irratti
if 'Maqaa Ogeessaa' in sheet_df.columns:
    hojjetaa = st.sidebar.selectbox(worker_label, sheet_df['Maqaa Ogeessaa'].dropna().unique())
    st.write(sheet_df[sheet_df['Maqaa Ogeessaa'] == hojjetaa])

# Sadarkaa madaallii agarsiisu (yoo kolumniin jiru ta’e)
if 'Sadarkaa irratti Madaalame' in sheet_df.columns:
    st.subheader("Sadarkaa Madaallii / Evaluation Rank")
    st.bar_chart(sheet_df['Sadarkaa irratti Madaalame'].value_counts())

# Form input itti dabaluun (evaluation entry)
with st.form("evaluation_form"):
    hojjetaa_input = st.text_input(worker_label)
    qabxii_input = st.number_input(score_label, min_value=0, max_value=100)
    submit = st.form_submit_button(submit_label)

    if submit:
        if lang == "Afaan Oromoo":
            st.success(f"{hojjetaa_input} → Qabxii {qabxii_input}% galmeessite")
        else:
            st.success(f"{hojjetaa_input} → Recorded score {qabxii_input}%")
