                ...
                for idx, row in res.iterrows():
                    with st.expander(f"📄 {row['Maqaa_Abbaa_Dhimmaa']}"):
                        ...
                        if st.button("💾 Update", key=f"btn_{idx}"):
                            ...
                            save_data(df)
                            st.rerun()
maa'].str.contains(q, case=False, na=False)]
            for idx, row in res.iterrows():
                with st.expander(f"📄 {row['Maqaa_Abbaa_Dhimmaa']}"):
                    ...
                    if st.button("Haqi", key=f"d_{idx}"):
                        df = df.drop(idx); save_data(df); st.rerun()MONTH_MAP = {9: "Fulbaana", 10: "Onkololeessa", 11: "Sadaasa", 12: "Muddee", 1: "Amajjii", 2: "Guraandhala", 3: "Bitootessa", 4: "Eebila", 5: "Caamsaa", 6: "Waxabajjii", 7: "Adooleessa", 8: "Hagayya"}
