...
# Maqaa fayila logo keetii asitti sirreessi
LOGO_PATH = "logo.png" 
...
if not st.session_state.logged_in:
    ...
    with col_mid:
        if os.path.exists(LOGO_PATH):
            st.image(LOGO_PATH, width=150) # Logo Seensaa
    ...
    with st.sidebar:
        if os.path.exists(LOGO_PATH):
            st.image(LOGO_PATH, width=120) # Logo Sidebar
    ...
    # --- DASHBOARD ---
    if menu == "📊 Dashboard":
        col_logo, col_txt = st.columns([0.1, 0.9])
        with col_logo:
            if os.path.exists(LOGO_PATH): st.image(LOGO_PATH, width=60)
        with col_txt:
