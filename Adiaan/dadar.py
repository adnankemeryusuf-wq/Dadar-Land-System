...
# ================= 4. MAIN APPLICATION =================
    ...
    # --- GABAASA CALALAME ---
        ...
        st.header("📈 Gabaasa fi Calalii")
        if not df.empty:
            col_f1, col_f2 = st.columns([1, 2])
            with col_f1:
                f_type = st.selectbox("Calali:", ["Waliigala", "Waggaa", "Kurmaana", "Ji'a", "Torbee", "Guyyaa Addaa"])
            ...
            if f_type == "Waggaa":
                ...
                y = st.selectbox("Waggaa", sorted(df['Waggaa'].unique(), reverse=True))
                q = st.radio("Kurmaana", [1,2,3,4], horizontal=True)
                ...
                filtered_df = df[df['Guyyaa'] == d.strftime('%d/%m/%Y')]
            ...
            if c2.button("✈️ Telegram-itti Ergi"):
                if send_to_telegram(buf.getvalue(), "Gabaasa.xlsx", f"📊 Gabaasa {f_type}: {t_f} ETB"):
