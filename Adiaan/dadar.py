...
# --- 2. CSS CUSTOM STYLE (MODERN & PREMIUM) ---
st.markdown("""
    <style>
    ...
    /* Header Box - Premium Gradient */
    .header-box { 
        text-align: center; padding: 60px 20px; 
        background: linear-gradient(135deg, #1e3a8a 0%, #2563eb 50%, #3b82f6 100%); 
        color: white; border-radius: 30px; margin-bottom: 40px;
        box-shadow: 0 20px 40px rgba(30, 58, 138, 0.2);
    }
    
    /* Metric Cards - Gold Border & Hover Effect */
    .metric-card {
    ...
    .metric-card h4 { color: #64748b; font-weight: 500; margin-bottom: 10px; font-size: 0.9rem; text-transform: uppercase; }
    .metric-card h2 { color: #1e3a8a; font-size: 2.3rem; font-weight: 800; }
...
def generate_certificate(name, rank, year):
    ...
    pdf.add_page()
    pdf.set_draw_color(30, 58, 138) 
    pdf.set_line_width(4); pdf.rect(10, 10, 277, 190)
    pdf.set_draw_color(212, 175, 55); pdf.set_line_width(1.5); pdf.rect(13, 13, 271, 184)
    if LOGO_PATH: pdf.image(LOGO_PATH, x=133, y=18, w=30)
    pdf.ln(45)
    pdf.set_font('Arial', 'B', 36); pdf.set_text_color(30, 58, 138)
    pdf.cell(0, 15, 'SARTIFIKETII KABAJAA', ln=True, align='C')
    pdf.ln(15); pdf.set_font('Arial', '', 20); pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 10, 'Badhaasni kun ogeessa gahumsa qabuuf kenname:', ln=True, align='C')
    pdf.ln(8); pdf.set_font('Arial', 'B', 32); pdf.set_text_color(184, 134, 11)
    pdf.cell(0, 20, name.upper(), ln=True, align='C')
    pdf.ln(10); pdf.set_font('Arial', '', 16)
    content = f"Waggaa {year} keessa tajaajila gaarii fi gahumsa qabuun hojjechuun badhaasa sadarkaa {rank}ffaa waan ta'aniif qophaa'e."
    pdf.multi_cell(0, 10, content, align='C')
    pdf.set_xy(180, 170); pdf.set_font('Arial', 'B', 14)
    pdf.cell(80, 7, "_______________________", ln=True, align='C')
    pdf.cell(260, 7, "Obbo Aqiil Abdujaliil", ln=True, align='R')
    return pdf.output(dest='S').encode('latin-1')
...
if not st.session_state.logged_in:
    ...
    with col:
        st.markdown('<div class="login-card">', unsafe_allow_html=True)
        if LOGO_PATH: st.image(LOGO_PATH, width=120)
        st.markdown("<h2 style='color:#1e3a8a; margin-top:15px;'>Dadar Land Admin System</h2>", unsafe_allow_html=True)
    ...
    with st.sidebar:
        if LOGO_PATH: st.image(LOGO_PATH, use_container_width=True)
        st.markdown("<h3 style='text-align:center; color:#1e3a8a;'>Dadar Land System</h3>", unsafe_allow_html=True)
        st.divider()
        menu = ["🏠 Dashboard", "📝 Galmee Haaraa", "📊 Gabaasa & Sartifiketii", "🚪 Logout"]
        choice = st.selectbox("Menu Filadhu", menu)
        st.info(f"📅 Hardha: {to_ethiopian(datetime.now())}")
    ...
    st.markdown(f"""
        <div class="header-box">
            <h1 style='margin:0; font-weight:800; font-size:2.8rem;'>Waajjira Lafaa Magaalaa Dadar</h1>
            <p style='font-size:1.4rem; opacity:0.9;'>Sistama Bulchiinsa Ragaa fi Galmee Ammayyaa</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if choice == "🏠 Dashboard":
        ...
        with tab1:
            if os.path.exists(DATA_FILE):
                ...
                st.download_button("📥 Excel Buufadhu", df_view.to_csv().encode('utf-8'), "Gabaasa.csv", "text/csv")
        with tab2:
            st.subheader("Sartifiketii Miidhagaa Uumi")
            c_name = st.text_input("Maqaa Ogeessaa")
            c_rank = st.selectbox("Sadarkaa Argatan", ["1ffaa", "2ffaa", "3ffaa"])
